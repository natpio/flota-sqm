...
st.markdown("""
    ...
    <div class="sqm-header">
        ...
        <p style="margin:0; opacity:0.8; font-size: 1.2rem;">Fleet Manager v9.1 (Label Fix)</p>
    </div>
...
# -----------------------------------------------------------------------------
# 6. RENDEROWANIE WYKRESÓW (FIXED LABELS)
# -----------------------------------------------------------------------------
if active_tab in RESOURCES:
    ...
    if not plot_df.empty:
        # Używamy prostego koloru dla 'event' by uniknąć problemów z legendą
        fig = px.timeline(
            plot_df, x_start="start", x_end="koniec", y="pojazd",
            color="event",
            category_orders={"pojazd": assets_to_show},
            template="plotly_white",
            text="event" # Deklarujemy tekst tutaj
        )
        
        fig.update_traces(
            textposition="inside", # Próbujemy wewnątrz
            insidetextanchor="start",
            textfont=dict(size=16, color="white", family="Inter"),
            # Jeśli się nie mieści, Plotly dzięki cliponaxis=False i poniższym ustawieniom powinno go pokazać
            cliponaxis=False 
        )

        # DODATKOWA WARSTWA: Jeśli tekst "zniknął", wymuszamy go jako adnotacje lub textmode
        fig.update_layout(
            uniformtext_minsize=14, 
            uniformtext_mode='show', # Wymusza pokazanie tekstu nawet jeśli się "nie mieści"
